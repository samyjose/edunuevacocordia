...
// --- DATOS DE EJEMPLO ---
// Como no hay una pantalla para gestionar estudiantes, usamos esta lista de ejemplo.
// En una aplicación real, esto vendría de la base de datos.
const MOCK_ESTUDIANTES = [
  { id: 'stu-1', nombre: 'Juan Perez', curso: '1ro BGU', paralelo: 'A' },
  { id: 'stu-2', nombre: 'Maria Lopez', curso: '1ro BGU', paralelo: 'A' },
  { id: 'stu-3', nombre: 'Carlos Sanchez', curso: '2do BGU', paralelo: 'B' },
  { id: 'stu-4', nombre: 'Ana Gomez', curso: '1ro BGU', paralelo: 'A' },
  { id: 'stu-5', nombre: 'Luis Fernandez', curso: '3ro BGU', paralelo: 'C' },
  { id: 'stu-6', nombre: 'Sofia Rodriguez', curso: '2do BGU', paralelo: 'B' },
];

// Obtenemos una lista única de cursos a partir de los estudiantes
const CURSOS_UNICOS = [...new Set(MOCK_ESTUDIANTES.map(e => e.curso))].sort();
...
function SistemaAsistenciaConcordia() {  
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      setCursos(data.cursos || [...new Set(MOCK_ESTUDIANTES.map(e => e.curso))].sort());
      setParalelos(data.paralelos || ['A', 'B', 'C', 'D', 'E']);
    }
  }, [currentUser]);
  ...
  // Función para añadir un nuevo paralelo
  const anadirParalelo = () => {
    const nuevoParalelo = window.prompt("Introduce la letra o nombre del nuevo paralelo:");
    if (nuevoParalelo && nuevoParalelo.trim() !== "") {
      const paraleloLimpio = nuevoParalelo.trim().toUpperCase();
      if (!paralelos.includes(paraleloLimpio)) {
        const paralelosActualizados = [...paralelos, paraleloLimpio].sort();
        setParalelos(paralelosActualizados);
        persistData({ paralelos: paralelosActualizados });
      } else {
        alert("Ese paralelo ya existe.");
      }
    }
  }

  // Filtra los estudiantes que pertenecen al curso seleccionado
  const estudiantesDelCurso = estudiantes.filter(e => e.curso === cursoSeleccionado);
  ...
  const registrarAsistenciaEstudiante = (estudiante, presente) => {
    ...
    const otrosRegistros = registro.filter(
      r => !(r.nombre === estudiante.nombre && r.fecha === hoy)
    );
  ...
  const getAsistenciaHoy = (estudiante) => {
    ...
    const registroHoy = registro.find(r => r.nombre === estudiante.nombre && r.fecha === hoy);
  ...
  const agregarEstudiante = (e) => {
    ...
    const paralelo = form.get("paralelo")?.toString().trim();
    const materia = form.get("materia")?.toString().trim();
    const fecha = form.get("fecha")?.toString();
    const asistencia = form.get("asistencia") === "true";

    if (!nombre || !paralelo || !materia || !fecha) {
            ...
            <Card className="p-4 shadow-md">
              ...
              <ul className="grid gap-3">
                {estudiantesDelCurso.map(estudiante => {
                  ...
                  return (
                    <li key={estudiante.id} className="p-3 bg-gray-50 rounded shadow-sm flex justify-between items-center">
          ...
          <Card className="p-4 shadow-md">
            <form
              ...
              onSubmit={(e) => {
                ...
                const paralelo = form.get("paralelo")?.toString();
                ...
                if (!nombre || !paralelo || !curso) return alert("Completa todos los campos");
                ...
                if (notaEditando) {
                  ...
                  actual = notas.map(n => 
                    n.id === notaEditando.id ? { ...notaEditando, nombre, paralelo, curso, p1, p2, p3, ex, sup, promedio } : n
                  );
                  setNotaEditando(null); // Salir del modo edición
                } else {
                  // Creando una nota nueva
                  const notaNueva = { id: genId(), nombre, paralelo, curso, p1, p2, p3, ex, sup, promedio };
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <select name="paralelo" className="border p-2 rounded w-full" required defaultValue={notaEditando?.paralelo}>
                  <option value="">Paralelo</option>
                  {paralelos.map(p => <option key={p} value={p}>{p}</option>)}
                </select>
                <Button type="button" onClick={anadirParalelo} className="w-auto px-4 text-sm">+</Button>
              </div>
              ...
              {notas.filter(n => n.nombre.toLowerCase().includes(filtroNotas.toLowerCase())).map((n) => (
                <li key={n.id} className="p-3 bg-white rounded shadow-sm">
                  <div className="flex justify-between items-start">
                    <span><b>{n.nombre}</b> - {n.curso} ({n.paralelo})</span>
                    <div className="flex gap-2">
          ...
          <Card className="p-4 shadow-md">
            <ul className="grid gap-2">
              {registro.map((r) => (
                ...
                >
                  <span>
                    <b>{r.nombre}</b> - {r.paralelo} - {r.materia} - {formatDate(r.fecha)}
                  </span>
                  <span
                    className={
